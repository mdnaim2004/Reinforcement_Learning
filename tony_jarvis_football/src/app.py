...
class FootballSimulation:
    ...
    def _estimate_ball_from_yolo(self, frame: np.ndarray) -> None:
        ...
        self.ball.radius = float(np.clip(radius, self.config.min_ball_radius, self.config.max_ball_radius))
    ...
    def _apply_gestures(self, gesture_output, hands, frame_shape) -> None:
        ...
        self.ball.radius = float(np.clip(self.ball.radius, self.config.min_ball_radius, self.config.max_ball_radius))

        self.ball.rotation_deg = (self.ball.rotation_deg + gesture_output.rotate_delta * self.config.rotate_speed) % 360
        ...
        if gesture_output.launch and self.ball.selected:
            self.ball.velocity = np.array([0.0, self.config.launch_velocity], dtype=np.float32)
            self.renderer.spawn_particles((float(self.ball.center[0]), float(self.ball.center[1])), count=24)
    ...
    def run(self) -> None:
        ...
        try:
            while True:
                ...
                gesture_output = self.gesture_engine.process(hands, frame.shape)

                self._apply_gestures(gesture_output, hands, frame.shape)
                if self.ball.selected:
                    self.renderer.spawn_particles((float(self.ball.center[0]), float(self.ball.center[1])), count=1)
                ...
                frame = self.renderer.draw_hud(frame, gesture_output.active_labels, fps)
                cv2.imshow("TONY.JARVIS Gesture Football", frame)
